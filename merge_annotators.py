...
def save_cleaned_annotations(clean_annotations, im_path, save_path):
    ...
    for bbox in clean_annotations:
        ...
        mat_annotations["annotations"][3].append(
            list(map(float, COLOR_ENCODING[bbox.class_name]))
        )
...
def combine_valid_bboxes(annotator1, annotator2, annotator3):
    ...
    for bbox1 in annotator1:
        ...
        if bbox1 not in processed:
            ...
            final_bbox = Bbox(
                *final_bbox_c.get_coords(), final_bbox_c.z_plane, final_class_name
            )
...
def main():
    ...
    samer_dir = (
        "/projectnb/npbssmic/ac25/Myelin_paper_CCP_dataset_samer/3rd_batch_annotations"
    )
    ...
    for arjun_file in os.listdir(arjun_dir):
        if arjun_file.endswith(".mat"):
            ...
            arjun_annotations = [
                item for sublist in Bbox.bboxes_unseen.values() for item in sublist
            ]
            Bbox.bboxes_unseen = {}
            ...
            anna_valid = get_valid(
                anna_annotations, arjun_annotations, samer_annotations
            )
            arjun_valid = get_valid(
                arjun_annotations, anna_annotations, samer_annotations
            )
            samer_valid = get_valid(
                samer_annotations, anna_annotations, arjun_annotations
            )
            ...
            save_cleaned_annotations(
                final_bboxes, "", save_path=save_dir + "/" + arjun_file
            )
